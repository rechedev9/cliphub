...
`GET /v1/errors` (admin) pages error events in commit order with an opaque
decimal `after=<cursor>`: pass the previous page's `next_after` while
`has_more` is true. Rows keep `received_at`, but it is not the order, since a
request stamps it before its commit. `GET /healthz` on the admin listener
