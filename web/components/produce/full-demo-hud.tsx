...
export function FullDemoHud({ options, map, onChange }: {
  ...
  return <FullDemoGroup title="HUD de la partida" note="Un HUD de retransmisión acompaña al POV del jugador.">
    <div className="grid min-w-0 items-start gap-4 sm:grid-cols-[minmax(0,1.6fr)_minmax(190px,1fr)]">
      <Dialog>
        <DialogTrigger asChild><button type="button" className="group relative min-w-0 overflow-hidden rounded border border-border-subtle text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" aria-label={`Ampliar HUD ${selected.name}`}><HudPreview theme={selected} map={map} /><span className="absolute bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded bg-black/80 px-2 py-1 text-meta text-white"><Expand className="size-3" aria-hidden />Ampliar</span></button></DialogTrigger>
      ...
      <div className="min-w-0 space-y-3">
        <p className="flex items-center gap-2 font-mono text-meta uppercase tracking-widest text-primary"><Palette className="size-4" aria-hidden /> HUD · 16:9</p>
