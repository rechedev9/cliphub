...
export function FullDemoMediaPreview({ asset, video = false, label, gain = 1, showDescription = true, preload = 'none' }: {
  asset: FullDemoAssetRef; video?: boolean; label: string; gain?: number; showDescription?: boolean; preload?: 'none' | 'metadata';
}): ReactNode {
  ...
  const props = { controls: true, preload, src: source, 'aria-label': label, onError: () => setFailedSource(source) };
  return <div className="w-full space-y-1.5">
    ...
    {showDescription ? <p className="text-body-sm text-fg-2">Previsualización del archivo original. La normalización y la mezcla se verifican al renderizar.</p> : null}
