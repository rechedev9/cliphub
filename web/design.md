...
- Command strip: 56px across the full content inset, carrying the sidebar toggle, the breadcrumb, the live job transport, an update control when a newer GitHub Release is available, and, while CS2 is recording, the magenta capture pip. The mobile bar is its narrow variant, not a separate header.
