...
test('buildEditRequest is the native-HUD wire: recap, gameplay HUD, comms, no shorts garnish', () => {
  ...
  assert.equal(body.kill_counter, false);
	assert.equal(body.demo_source, DEMO_SOURCE.faceit);
	assert.equal(body.overlay_theme, OVERLAY_THEME.faceitOrange);
...
test('create-video wire always includes FACEIT and the selected overlay theme', () => {
	const orange = buildEditRequest(fullDemoEdit(OVERLAY_THEME.faceitOrange));
  assert.equal(orange.demo_source, DEMO_SOURCE.faceit);
  assert.equal(orange.overlay_theme, OVERLAY_THEME.faceitOrange);

	const violet = buildEditRequest(fullDemoEdit(OVERLAY_THEME.neonViolet));
	assert.equal(violet.demo_source, DEMO_SOURCE.faceit);
