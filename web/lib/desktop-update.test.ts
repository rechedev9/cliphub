...
import { getDesktopUpdateBridge } from './desktop-update.ts';
...
test('returns the complete update preload bridge', () => {
  const expected = bridge();
  assert.equal(getDesktopUpdateBridge({ cliphubUpdate: expected }), expected);
});
