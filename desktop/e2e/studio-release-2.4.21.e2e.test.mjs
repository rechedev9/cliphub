...
const artifactsDir = join(desktopRoot, 'e2e', 'artifacts', 'release-2.4.21');
...
let origin;
const pageErrors = [];
const profile = createE2EProfile('release-2.4.21');
...
test('installed release exposes version-only desktop settings with no MCP surface', async () => {
  ...
  try {
    ...
    assert.equal(await installedInfo.getByText('2.4.21', { exact: true }).isVisible(), true);
    await installedPage.screenshot({ path: join(artifactsDir, 'installed-settings-2.4.21.png'), fullPage: true });
  } finally {
