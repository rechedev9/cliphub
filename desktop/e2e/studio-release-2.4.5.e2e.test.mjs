// Release evals for FragForge Studio 2.4.5. This launches the real Electron
...
test('installed release exposes version-only desktop settings with no MCP surface', async () => {
  ...
  try {
    ...
    assert.equal(await installedInfo.getByText('2.4.5', { exact: true }).isVisible(), true);
    await installedPage.screenshot({ path: join(artifactsDir, 'installed-settings-2.4.5.png'), fullPage: true });
  } finally {
