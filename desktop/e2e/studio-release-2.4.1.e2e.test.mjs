// Release evals for FragForge Studio 2.4.1. This launches the real Electron
...
const artifactsDir = join(desktopRoot, 'e2e', 'artifacts', 'release-2.4.1');
...
test('installed release exposes version-only desktop settings with no MCP surface', async () => {
  ...
  try {
    ...
    assert.equal(values[versionIndex], '2.4.1');
    await installedPage.screenshot({ path: join(artifactsDir, 'installed-settings-2.4.1.png'), fullPage: true });
  } finally {
