// Release evals for TickCut Studio 2.4.14. This launches the real Electron
...
const artifactsDir = join(desktopRoot, 'e2e', 'artifacts', 'release-2.4.14');
...
let origin;
const pageErrors = [];
const profile = createE2EProfile('release-2.4.14');
...
test('installed release exposes version-only desktop settings with no MCP surface', async () => {
  ...
  try {
    ...
    assert.equal(await installedInfo.getByText('2.4.14', { exact: true }).isVisible(), true);
    await installedPage.screenshot({ path: join(artifactsDir, 'installed-settings-2.4.14.png'), fullPage: true });
  } finally {
