...
test('Local Studio assigns suspended services before resuming them', () => {
  ...
  assert.equal(
    launcher.match(/\[FragForge\.LocalProcessJob\]::StartInJob\(/g)?.length,
    2,
  );
...
$job = [FragForge.LocalProcessJob]::CreateKillOnClose()
$process = $null
try {
    $process = [FragForge.LocalProcessJob]::StartInJob(
        $job,
        $command,
        "/d /c exit 37",
        $RunRoot
    ...
    [FragForge.LocalProcessJob]::Close($job)
}
`, 'utf8');
...
for ($iteration = 0; $iteration -lt 10; $iteration++) {
    ...
    $job = [FragForge.LocalProcessJob]::CreateKillOnClose()
    try {
        [void][FragForge.LocalProcessJob]::StartInJob(
            $job,
            $powershell,
            $arguments,
            $iterationRoot
