...
test('reuses complete cached installations without download work', async (t) => {
  ...
  const paths = {
    hlae: path.join(toolsDir, 'hlae', '2.192.2-cliphub.1', 'HLAE.exe'),
    ffmpeg: path.join(
      toolsDir,
      'ffmpeg',
  ...
  writeCompleteMarker(path.join(toolsDir, 'hlae', '2.192.2-cliphub.1'), 'hlae');
  ...
  assert.equal(fs.existsSync(path.join(toolsDir, 'hlae', '2.192.2-cliphub.1', '.cliphub-install.json')), true);
...
test('retires markerless legacy tools instead of using them after a failed refresh', async (t) => {
  ...
  assert.equal(fs.existsSync(path.join(toolsDir, 'hlae', '2.192.2-cliphub.1', '.cliphub-install.json')), false);
...
test('installs uncached tools through staging and publishes only complete versions', async (t) => {
  ...
  const versions = { hlae: '2.192.2-cliphub.1', ffmpeg: 'n8.1.2-30-g45f1910444-20260723', ytdlp: '2026.07.04' };
  for (const [name, version] of Object.entries(versions)) {
    ...
    assert.equal(fs.existsSync(path.join(installDir, '.cliphub-install.json')), true);
    const marker = JSON.parse(fs.readFileSync(path.join(installDir, '.cliphub-install.json'), 'utf8'));
...
test('caller cancellation aborts work instead of activating legacy fallbacks', async (t) => {
  ...
  for (const [name, version] of Object.entries({ hlae: '2.192.2-cliphub.1', ffmpeg: 'n8.1.2-30-g45f1910444-20260723', ytdlp: '2026.07.04' })) {
...
function digestFor(url: string): string {
  if (url.includes('advancedfx')) {
    return 'b27d92058ee988d4da5cae60148fe437378d3229eec70f64e7ecdf0b0c2ddf19';
  }
...
function seedLegacyHLAE(toolsDir: string): void {
  const executable = path.join(toolsDir, 'hlae', '2.192.2-cliphub.1', 'HLAE.exe');
  fs.mkdirSync(path.dirname(executable), { recursive: true });
  fs.writeFileSync(executable, 'cached');
}
...
function collectFixtureFiles(directory: string, relativeDirectory = ''): string[] {
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    if (entry.name === '.cliphub-install.json') continue;
...
function fixtureTreeSha256(toolsDir: string): Partial<Record<RuntimeToolName, string>> {
  const directories: Record<RuntimeToolName, string> = {
    hlae: path.join(toolsDir, 'hlae', '2.192.2-cliphub.1'),
