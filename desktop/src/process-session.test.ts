...
test('can remove a sensitive inherited variable for a specific child', (t) => {
  const previous = process.env.CLIPHUB_TEST_SECRET;
  process.env.CLIPHUB_TEST_SECRET = 'inherited-secret';
  t.after(() => {
    if (previous === undefined) delete process.env.CLIPHUB_TEST_SECRET;
    else process.env.CLIPHUB_TEST_SECRET = previous;
  });
  ...
  session.launch('web', 'node.exe', [], { CLIPHUB_TEST_SECRET: undefined });

  assert.equal(childEnvironment?.CLIPHUB_TEST_SECRET, undefined);
  assert.equal(childEnvironment && 'CLIPHUB_TEST_SECRET' in childEnvironment, false);
});
...
test('removes differently-cased sensitive variables from Windows child environments', (t) => {
  ...
  const inheritedName = 'CLIPHUB_test_secret';
  ...
  const inheritedKeys = Object.keys(childEnvironment ?? {})
    .filter((name) => name.toLowerCase() === 'CLIPHUB_test_secret');
