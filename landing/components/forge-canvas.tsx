 ...
 * thousands of orange sparks (a rare violet ember among them) rising and
 ...
 * y=-5.3, so the peak luminance sits below the fold and only a faint ember
...
void main() {
  ...
  // Color ramp: brand orange -> white-hot (linear space; sRGB output applied
  // by the composer). Keeps peak brightness low and off the headline zone.
  // A rare (~1.5%) ember runs violet instead, the second brand tone, among a
  // field that otherwise reads entirely orange.
  vec3 ember = vec3(0.984, 0.573, 0.235);
  vec3 violet = vec3(0.545, 0.361, 0.965);
  vec3 base = mix(ember, violet, step(0.985, aSeed));
...
void main() {
  ...
  float a = vAlpha * core;
  // Hottest cores exceed the bloom threshold; cool sparks stay pure orange
  // (or violet for the rare tinted ember).
...
void main() {
  ...
  vec3 ember = vec3(0.78, 0.42, 0.12);
  gl_FragColor = vec4(ember * glow * pulse * 0.4, glow * 0.3);
}
`;
