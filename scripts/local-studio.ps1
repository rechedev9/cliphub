...
# .\scripts\local-studio.ps1 -OrchestratorPort 18080 -WebPort 13000 -DataDir C:\temp\cliphub-qa
# Browser automation should also pass -NoBrowser, provide its own
# CLIPHUB_PROXY_BOOTSTRAP_CAPABILITY, and navigate its browser session to
...
#   - Node.js + pnpm 11.22.0. If node_modules is missing, the script installs
#     exactly the dependency graph in pnpm-lock.yaml.
#   - CS2 + an HLAE build compatible with the installed CS2: the Studio pin from
#     desktop/src/hlae-tool.json (%APPDATA%\cliphub-studio\tools\hlae\<version>\HLAE.exe)
#     or an official release under C:\HLAE-<version>\HLAE.exe. Capture needs them;
...
$mutatedEnvironmentNames = @(
    ...
    "CLIPHUB_PROXY_BOOTSTRAP_CAPABILITY",
    "PORT",
    "HOSTNAME"
)
$originalEnvironment = @{}
...
$proxyCapability = if ([string]::IsNullOrWhiteSpace($originalEnvironment["CLIPHUB_PROXY_MUTATION_CAPABILITY"])) {
    New-LocalCapability
} else {
    $originalEnvironment["CLIPHUB_PROXY_MUTATION_CAPABILITY"]
}
$bootstrapCapability = if ([string]::IsNullOrWhiteSpace($originalEnvironment["CLIPHUB_PROXY_BOOTSTRAP_CAPABILITY"])) {
    if ($NoBrowser) {
        throw "-NoBrowser requires CLIPHUB_PROXY_BOOTSTRAP_CAPABILITY so the caller can complete the browser handshake"
    }
    New-LocalCapability
} else {
    $originalEnvironment["CLIPHUB_PROXY_BOOTSTRAP_CAPABILITY"]
}
...
try {
$processJob = [ClipHub.LocalProcessJob]::CreateKillOnClose()
...
try {
    ...
    $orchestrator = [ClipHub.LocalProcessJob]::StartInJob(
        $processJob,
        $binOrchestrator,
        "",
        $repoRoot
...
try {
    ...
    if ($processJob -ne [IntPtr]::Zero) {
        [ClipHub.LocalProcessJob]::Close($processJob)
    }
    foreach ($name in $mutatedEnvironmentNames) {
