...
- **Hosted CI** (PRs and `main`): `CI frontend` (web + desktop typecheck/lint/unit), `CI backend` (`go vet`, `go test ./...`, `zv check`, Node simulator contracts; FFmpeg media canaries must pass without skips; see `docs/CAPTURE_LAB.md`; triggers on `CLAUDE.md`, `AGENTS.md`, `.claude/**` too), `CI infra` (actionlint + release contract). The `Go 1.26.6` pin in this file is checked by `desktop/scripts/go-toolchain.test.mjs` under `pnpm --dir desktop run test:unit` (`CI frontend`). Not HLAE/CS2 E2E, not Playwright. `main` is unprotected with no required checks.
