...
- `full-demo-rail` — Sidebar `03 Demo completa` → `/full-demo`.
...
Header **DEMO COMPLETA A VÍDEO**. Contract: landscape 16:9, native HUD, comms, no music. A landscape recap exists in Biblioteca with PARTIDA COMPLETA. Hosted CI green is not this.
...
- **UI.** Click **Demo completa**. Contract rows visible. Picker only lists parsed matches.
