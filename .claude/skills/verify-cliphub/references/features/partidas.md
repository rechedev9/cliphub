...
- `partidas-rail` — Sidebar `01 Partidas` → `/matches`.
...
The list shows local jobs. A series shares a client-minted `series_id`. Opening a match shows roster and next actions (Shorts, Demo completa) without inventing remote match data. Empty copy: **Aún no hay partidas**.
...
- **Empty.** No jobs: title Aún no hay partidas, action EMPIEZA AQUÍ → `/onboarding`.
