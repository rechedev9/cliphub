...
func TestRunVerifyDoctorJSONSchemaAndGap(t *testing.T) {
	...
	if report.Host.CaptureRecertification != verify.CaptureUnavailable {
		return
	}
	if report.OK || !report.Closed {
		t.Fatalf("unavailable capture must not be ok: %#v", report)
	}
	if runtime.GOOS != "windows" && !named {
		t.Fatalf("linux unavailable capture must name hlae_cs2_windows_studio: %#v", report)
	}
	if runtime.GOOS == "windows" && len(report.Gaps) == 0 {
		t.Fatalf("windows unavailable capture must name a closed gap: %#v", report)
	}
}
