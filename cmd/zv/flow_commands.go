	...
	return []productionFlow{
		{
			...
			Phases: []flowPhase{
				...
				{ID: "full-demo-brief", Goal: "review the complete document, assets, round ranges, timeline and blockers", Command: "zv full-demo inspect --plan <plan.json> --format json", When: "full-demo-pov-chill-v1", Gate: true, ReadOnly: true, Decision: "approve this plan hash and grant the current HLAE/CS2 run"},
				{ID: "full-demo-execute", Goal: "enqueue the same capture/render workers as Studio with immutable approval", Command: "zv full-demo execute --job <uuid> --plan <plan.json> --approve <plan-hash> --allow-safe-tail-trim=true --dry-run --format json", When: "full-demo-pov-chill-v1", Decision: "use the document's explicit safety-tail value; remove --dry-run only after the shown brief and hardware approval", Gate: true, Produces: "generate intent and current render revision"},
