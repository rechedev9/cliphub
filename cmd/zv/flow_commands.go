	...
	return []productionFlow{
		{
			...
			Source:               ".dem",
			RequiredArtifacts:    []string{"demo path"},
			...
			SafetyGates:          []string{"target player selection", "live HLAE/CS2 capture approval", "long FFmpeg render approval", "thumbnail selection when covers are enabled"},
			...
			Phases: []flowPhase{
				...
				{ID: "full-demo-import", Goal: "import the selected player into the existing job/parser queue", Command: "zv full-demo import --demo <match.dem> --steamid <SteamID64> --dry-run --format json", Decision: "validate inputs, then upload without --dry-run and inspect job status", When: "full-demo-pov-chill-v1", Produces: "job and independent round facts"},
				{ID: "full-demo-options", Goal: "inspect all explicit capture, audio, sponsor and output choices", Command: "zv full-demo defaults --format json", When: "full-demo-pov-chill-v1", ReadOnly: true},
				{ID: "full-demo-plan", Goal: "persist a document against current facts, team voice and declared asset bytes", Command: "zv full-demo plan --job <uuid> --options <options.json> --out <plan.json> --format json", When: "full-demo-pov-chill-v1", Produces: "immutable Full Demo plan; no capture"},
				{ID: "full-demo-plan-review", Goal: "inspect the complete document, assets, round ranges, timeline and blockers", Command: "zv full-demo inspect --plan <plan.json> --format json", When: "full-demo-pov-chill-v1", ReadOnly: true, Decision: "validate the saved plan hash and required assets"},
				{ID: "full-demo-execute", Goal: "enqueue the same capture/render workers as Studio with an immutable plan", Command: "zv full-demo execute --job <uuid> --plan <plan.json> --approve <plan-hash> --allow-safe-tail-trim=true --dry-run --format json", When: "full-demo-pov-chill-v1", Decision: "bind the saved plan hash and its explicit safety-tail value; remove --dry-run only for an authorized hardware run", Gate: true, Produces: "generate intent and current render revision"},
				...
				{ID: "edit-preflight", Goal: "validate the selected render configuration without rendering", Command: "zv shorts render --recording-result <run>/recording/recording-result.json --killplan <run>/selected-plan.json --out <run>/render --publish-dir <run>/shortslistosparasubir --preset viral-60-clean --output-format <format> --kill-effect <effect> --transition <transition> --hook=<true|false> --kill-counter=<true|false> --intro=<true|false> --outro=<true|false> [--intro-text <text>] [--outro-text <text>] [--music <track>] --covers=<true|false> --cover-sheets=<true|false> --compile-segments --dry-run", Decision: "inspect exact render argv", Produces: "editor dry-run manifests and publish metadata", ReadOnly: false},
				{ID: "edit", Goal: "render one polished compilation with the selected render configuration", Command: "zv shorts render --recording-result <run>/recording/recording-result.json --killplan <run>/selected-plan.json --out <run>/render --publish-dir <run>/shortslistosparasubir --preset viral-60-clean --output-format <format> --kill-effect <effect> --transition <transition> --hook=<true|false> --kill-counter=<true|false> --intro=<true|false> --outro=<true|false> [--intro-text <text>] [--outro-text <text>] [--music <track>] --covers=<true|false> --cover-sheets=<true|false> --compile-segments", Produces: "candidate pack when covers are enabled; upload-ready pack when covers are disabled", Expensive: true},
		...
		},
		{
			...
			SafetyGates:          []string{"valid clip bounds/title", "crop/framing reviewed", "source-audio treatment", "long FFmpeg render approval", "third-party music provenance when music is supplied"},
			DryRunBehavior:       "stream dry-runs validate/probe but do not create the --out edit plan artifact; persist the configured plan before dependent render stages",
			LiveBehavior:         "renders the persisted edit plan into a local upload-ready pack without ad hoc FFmpeg flags outside the plan",
			ResumePolicy:         "changing the persisted stream plan requires a fresh render using the updated plan",
			...
			Phases: []flowPhase{
				...
				{ID: "plan", Goal: "persist the configured stream edit contract", Command: "zv stream plan --input <stream.mp4> --out <run>/edit-plan.json --variant <" + strings.Join(streamclips.VariantNames(), "|") + "> --format json", Produces: "edit-plan.json"},
				{ID: "plan-review", Goal: "inspect the persisted edit plan and settle any change before render", Command: "zv stream render --input <stream.mp4> --plan <run>/edit-plan.json --out <run>/render --dry-run --format json", Decision: "inspect clip ranges, order, crop, source audio, fades, text, and music volume", ReadOnly: true},
