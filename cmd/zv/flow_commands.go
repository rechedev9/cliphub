	...
	return []productionFlow{
		{
			...
			Phases: []flowPhase{
				...
				{ID: "edit-preflight", Goal: "validate the approved creative brief without rendering", Command: "zv shorts render --recording-result <run>/recording/recording-result.json --killplan <run>/selected-plan.json --out <run>/render --publish-dir <run>/shortslistosparasubir --preset viral-60-clean --output-format <approved-format> --kill-effect <approved-effect> --transition <approved-transition> --kill-counter=<true|false> --intro=<true|false> --outro=<true|false> [--intro-text <text>] [--outro-text <text>] [--music <track>] --covers=<true|false> --cover-sheets=<true|false> --compile-segments --dry-run", Decision: "approve exact render argv", Produces: "editor dry-run manifests and publish metadata", ReadOnly: false},
				{ID: "edit", Goal: "render one polished compilation with the approved creative brief", Command: "zv shorts render --recording-result <run>/recording/recording-result.json --killplan <run>/selected-plan.json --out <run>/render --publish-dir <run>/shortslistosparasubir --preset viral-60-clean --output-format <approved-format> --kill-effect <approved-effect> --transition <approved-transition> --kill-counter=<true|false> --intro=<true|false> --outro=<true|false> [--intro-text <text>] [--outro-text <text>] [--music <track>] --covers=<true|false> --cover-sheets=<true|false> --compile-segments", Produces: "candidate pack when covers are enabled; upload-ready pack when covers are disabled", Expensive: true},
		...
		},
		{
			...
			Phases: []flowPhase{
				...
				{ID: "plan-preflight", Goal: "probe media and preview the clip/crop contract without writing", Command: "zv stream plan --input <stream.mp4> --out <run>/edit-plan.json --variant <" + strings.Join(streamclips.VariantNames(), "|") + "> --dry-run --format json", Decision: "clip ranges and clean crops", ReadOnly: true, Expensive: true},
				{ID: "plan", Goal: "persist the approved stream edit contract", Command: "zv stream plan --input <stream.mp4> --out <run>/edit-plan.json --variant <" + strings.Join(streamclips.VariantNames(), "|") + "> --format json", Produces: "edit-plan.json", Expensive: true},
