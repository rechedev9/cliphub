...
func TestStoreFullDemoSteamAvatarsSnapshotsPublicAndPrivateWithoutFACEITFields(t *testing.T) {
	...
	if !got[privateID].Private || got[privateID].URL != "https://avatars.akamai.steamstatic.com/private.jpg" {
...
type fakeSteamAvatarResolver struct{ avatars map[string]faceit.SteamAvatar }

func (f fakeSteamAvatarResolver) ResolveSteamAvatars(_ context.Context, _ []string) map[string]faceit.SteamAvatar {
	return f.avatars
}
