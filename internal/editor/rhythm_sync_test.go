...
func TestIndexRhythmSyncRejectsEmptySegmentSync(t *testing.T) {
	_, err := indexRhythmSync(rhythm.Analysis{SchemaVersion: "1.0"})
	if err == nil {
		t.Fatal("indexRhythmSync returned nil error, want error")
	}
...
func TestIndexRhythmSyncRejectsMissingSegmentID(t *testing.T) {
	_, err := indexRhythmSync(rhythm.Analysis{
		SchemaVersion: "1.0",
		SegmentSync:   []rhythm.SegmentSync{{TimelineStartSeconds: 0.5}},
	})
	if err == nil {
		t.Fatal("indexRhythmSync returned nil error, want error")
	}
