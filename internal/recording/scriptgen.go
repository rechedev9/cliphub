...
func generateHLAEJavaScript(plan RecordingPlan, attestationToken string) (string, error) {
	...
	sb.WriteString("                    // Stall watchdog: a seek in flight must advance the demo tick. If\n")
	sb.WriteString("                    // the observed tick makes no progress for a budget of consecutive\n")
	sb.WriteString("                    // frames, the seek is wedged and re-issuing demo_gototick will not\n")
	sb.WriteString("                    // recover it. Reset the counter whenever the tick advances or a new\n")
	sb.WriteString("                    // seek begins. Frame counting only, never wall-clock time.\n")
	...
	sb.WriteString("                            failCapture(`seek ${seekIdx + 1} stalled at target ${s.target}: demo tick made no progress for ${maxSeekStallFrames} consecutive frames`);\n")
	sb.WriteString("                            return;\n")
	sb.WriteString("                        }\n")
	sb.WriteString("                    }\n")
