...
Cloud Linux weekday maintain of `.cursor/skills/verify-cliphub/` on 2026-09-15 against current main (after #187). The instance served `http://127.0.0.1:4173`. Capture is closed.
...
- `upload.aria.txt` heading `Crea un Short`. `Elegir demos de CS2` is disabled (no orchestrator).
- `full-door.aria.txt` heading `Crea un vídeo largo`. Format-bar `Vídeo largo 16:9` is pressed.
- `streams.aria.txt` is the settled import form (no `Cargando streams`). URL field `Enlace del vídeo de Twitch, YouTube o Kick`.
