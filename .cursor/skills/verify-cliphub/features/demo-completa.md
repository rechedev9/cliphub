...
- After a parsed POV the form has five groups: `HUD de la partida` (not the brief chip `HUD`), `Sonido` (toggle `Incluir voces del equipo` plus gain sliders `Juego` / `Voces`, added with the 8dd0aa93 layout/audio pass), `Entre rondas` (transitions; added with the #187 simplify), `Overlays`, and `Sponsor`.
