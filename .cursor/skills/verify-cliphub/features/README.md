...
- Run browser actions through `control-cliphub.mjs`. `click` on a link waits for the destination URL. `goto` of `/clips/nueva` identifies ClipHub from the brand lockup when the tab title is only `Cargar demo`.
