...
- **Empty first-run.** Look for the empty region. Run `node .cursor/skills/verify-cliphub/control-cliphub.mjs snapshot --out .cursor/skills/verify-cliphub/artifacts/inicio/hub.aria.txt`. The snapshot contains `¿Qué quieres crear?` or, when jobs exist, `Tus demos y vídeos`.
